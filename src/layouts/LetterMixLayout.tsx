      ...
      <header className="px-4 py-0 flex items-center justify-between sticky top-0 z-30"
        ...
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-base font-black tracking-wide flex items-center gap-2"
            ...
            <span style={{ color: 'var(--lm-accent)' }}>Clear the String</span>
            ...
            {menuOpen && (
              <motion.nav initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}
                ...
                <div className="p-1.5 flex flex-col gap-0.5">
                  <button onClick={() => { if (resetHandler) resetHandler(); setMenuOpen(false); }}
                    className="w-full text-left px-3 py-2 rounded text-sm font-bold" style={{ color: '#141414' }}
                    onMouseEnter={e => { e.currentTarget.style.background = '#d9f24b'; }}
                    onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; }}>
                    Reset puzzle
                  </button>
                  <Link to="/lettermix/calendar" onClick={() => setMenuOpen(false)}
                ...
                <div className="px-3 py-2" style={{ borderTop: '2px solid #141414', borderBottom: '2px solid #141414' }}>
                  <span className="text-xs font-black uppercase tracking-widest" style={{ color: '#6f6a5e', fontFamily: "'JetBrains Mono', monospace" }}>Info</span>
                </div>
                <div className="p-1.5">
                  <Link to="/lettermix/about" onClick={() => setMenuOpen(false)}
                    className="block px-3 py-2 rounded text-sm font-bold" style={{ color: '#141414', textDecoration: 'none' }}
                    onMouseEnter={e => { e.currentTarget.style.background = '#d9f24b'; }}
                    onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; }}>
                    About
